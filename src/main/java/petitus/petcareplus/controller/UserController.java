...
@Tag(name = "User", description = "APIs for your user account")
...
public class UserController extends BaseController {
    ...
    @PutMapping
    @Operation(tags = {"User"}, summary = "Update current user", description = "API để cập nhật thông tin người dùng hiện tại")
    public ResponseEntity<UserResponse> updateCurrentUser(@RequestBody @Valid final UpdateUserRequest user) throws BindException {
        return ResponseEntity.ok(UserResponse.convert(userService.update(userService.getCurrentUserId().toString(), user)));
    }

    @GetMapping
    @Operation(tags = {
            "User" }, summary = "Get all users", description = "API để toàn bộ người dùng", security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @GetMapping("/{id}")
    @Operation(tags = {
            "User" }, summary = "Get an user", description = "API để lấy thông tin một người dùng", security = {
            @SecurityRequirement(name = "bearerAuth") })
